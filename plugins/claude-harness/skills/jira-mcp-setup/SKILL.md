...
name: jira-mcp-setup
description: >
  统一的 Jira 问题处理中心。提供：ensureConnected() 确保 MCP 可用、
  getIssue() 获取并处理 Jira ticket、writeBack() 回写结果到 Jira。
  被 autopilot/hotfix 调用，所有 Jira 相关逻辑集中在此。
---
...
1. 解析 URL 提取 issueKey
   ...
   issueKey = 匹配到的 key（如 TRNSCN-2539）
